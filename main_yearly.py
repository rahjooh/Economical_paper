...
def L(E_t, S_t, landa, mu, i, G, R_f, R_s, R_p, t_j, v_j):
    """
...
if __name__ == "__main__":
    ...
    data = [] ; best = []
    data.append( 'lagranzh' +','+ 'Lambda' +','+  'mu'+','+  't_j'+','+ 'v_j'+','+ 't_j*(E_t-R_p)'+','+ 'v_j*(E_t-R_s)'+','+ 'year'+'\n\r')
    for j in range(len(year)):
        for i in range(c.iteration):
            ...
            Lag = L(E_t=Et, S_t=St, landa=landa, mu=mu, i=j, G=G, R_f=Rf, R_s=Rs, R_p=Rp, t_j=r1, v_j=r2)
            if result < Lag:
                if r1 * (Et[j] - Rp[j]) <= 0 : continue
                if r2 * (Et[j] - Rs[j]) <= 0 : continue
                result = Lag
                print(result, '\t\t landa=', landa, '\t mu=', mu, '\t t_j=', r1, '\t  v_j=', r2)
                best =''+str(Lag)+','+  str(landa)+','+  str(mu)+','+  str(r1)+','+  str(r2)+','+  str(r1 * (Et[j] - Rp[j]))+','+  str(r2 * (Et[j] - Rs[j]))+','+  str(year[j])+'\n\r'
        data.append(best)
